...
string HeadExamination(string head,string fromUpLayer) {
    ...
    if (fromUpLayer.size() % 16 == 0) {
        ...
        while (bytes.size() > 1) {
            ...
            bytes.pop_back();
            temp.clear();
            temp = Tools::bmoc_calculation(A, B);
            bytes.push_back(temp);
        }
        result = bytes[0];
        for (int i = 0; i < 16; i++) {
...
//    port_des = buf; //目的端口
    port_src = Tools::transfer(basisInformation.srcPort, 16);//源端口
	port_des = Tools::transfer(basisInformation.desPort, 16); //目的端口
    serial_number = Tools::transfer(0,64) ;//序号
    confirm_number =Tools::transfer(0,64) ;//确认号
    data_offset = Tools::transfer(5,4);//数据偏移
    keep = Tools::transfer(0,6);//保留
    urg = Tools::transfer(0, 1);//紧急urg
    ack = Tools::transfer(1, 1);//确认ack
    psh = Tools::transfer(1, 1);//推送psh
    rst = Tools::transfer(0, 1);//复位rst
    syn = Tools::transfer(1, 1);//同步syn
    fin = Tools::transfer(0, 1);//终止fin
    window = Tools::transfer(0, 16);//窗口
    head_examination = Tools::transfer(0, 16);//首部检验和
    urgent_pointer = Tools::transfer(0, 16);//紧急指针
    option = Tools::transfer(0, 0);//选项
    sourceIP = "";
    destinationIP = "";
    for (int i = 0; i < NumOfIP; i++) {
        sourceIP.append(Tools::transfer(atoi(basisInformation.srcIP[i].c_str()), 8));
        destinationIP.append(Tools::transfer(atoi(basisInformation.desIP[i].c_str()), 8));
    }
    ...
    exam = sourceIP + destinationIP+Tools::transfer(0,8)+Tools::transfer(6,8) + Tools::transfer(static_cast<int>(fromUpLayer.length()) / 2, 16)+ result.head + fromUpLayer;
    ...
    result.head = Tools::binToHex(result.head);
    return result;
}
...
vector <DataFormat> ProcessTCP:: interpret(const Data info) {
    ...
    string tcp_head;
    tcp_head = Tools::hexToBin(info.head);
...
//     ProcessTCP p;
//     Data data = p.encapsulate("12345");

//     vector <DataFormat> test = p.interpret(data);
//     int l = test.size();
//     cout << l << endl;
//     for(int i = 0; i < l; ++i) {
//         cout << test[i].valueOfField << "  " << test[i].explainOfField << endl;
//     }
